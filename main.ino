...
// ===== CHÂN PHẦN CỨNG (CYD 2.8") =====
#define TFT_BL_PIN    21   // Đèn nền màn hình
#define PIN_HEATER    17   // Pad LED1 Blue (Kích chân 1 Anode của EL3063)
#define PIN_FAN       16   // Pad LED1 Green (PWM Mosfet Quạt)
...
// ===== CẤU HÌNH LOGIC ACTIVE-HIGH CHO CHÂN 1 OPTO =====
#define HEATER_ON     HIGH  // Xuất 3.3V -> Sáng LED Opto -> Bật PTC
#define HEATER_OFF    LOW   // Xuất 0.0V -> Tắt LED Opto   -> Ngắt PTC
...
void applyFanState() {
  int duty = 0;
  if (isCoolingDown) {
    duty = 255; // Quạt chạy 100% khi xả nhiệt sau khi dừng
  } else if (isDrying || (heaterDutyPercent > 0.0)) {
    duty = 255; // Ép quạt chạy 100% khi gia nhiệt
  } else if (fanEnabled) {
...
void updateHeaterControl() {
  ...
  // 1. FAILSAFE: Cảm biến ngắt kết nối / Lỗi dữ liệu -> Ngắt sưởi lập tức
  ...
  if (manualPwmMode) {
    heaterDutyPercent = constrain(manualDutyPercent, 0.0f, PTC_MAX_CAP);
  } else {
    ...
    // Khi chạm ngưỡng Target: Ngắt PTC ngay, quạt vẫn thổi
    ...
    if (isOverheatCooling) {
      ...
      if (error <= 0.0) {
        ...
        heaterDutyPercent = PTC_MAX_CAP; // Khóa công suất đỉnh 40%
      } else if (error >= 1.0) {
        heaterDutyPercent = 20.0;
      } else {
        heaterDutyPercent = 10.0;
      }
    }
  }

  // Khóa liên động quạt
  if (heaterDutyPercent > 0.0) {
    applyFanState();
  }
...
<body>
  <div class="wrapper">
    ...
    <div class="card">
      ...
      <!-- BẢNG BĂM XUNG PTC HEATER POWER (Khóa trần 40%) -->
      <div class="pwm-control-box">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <span style="font-weight:600; font-size:0.8rem;">PTC Heater Power (Capped at 40%)</span>
        ...
        <input type="range" id="pwmSlider" class="slider" min="0" max="40" value="0" oninput="updatePwmLabel(this.value)" onchange="sendPwmDuty(this.value)">
      </div>
...
<script>
  ...
  function setPwmModeState(enable) {
    ...
    if (enable && Number(val) === 0) {
      val = 30;
      document.getElementById('pwmSlider').value = 30;
    }
...
void drawK1AdjustPage() {
  ...
  bool fanIsVisualOn = (fanEnabled && !isOverheatCooling) || isCoolingDown || isDrying || (heaterDutyPercent > 0.0);
...
void handleStatus() {
  ...
  json += "\"fan\":" + String(((fanEnabled && !isOverheatCooling) || isCoolingDown || isDrying || (heaterDutyPercent > 0.0)) ? "true" : "false") + ",";
...
void handleSetPwm() {
  ...
  if (server.hasArg("duty")) {
    manualDutyPercent = constrain(server.arg("duty").toFloat(), 0.0f, PTC_MAX_CAP);
...
void setup() {
  ...
  // Khởi tạo chân ngắt sưởi lập tức
  ...
  // Cấu hình Watchdog Timer 5 giây
  #if defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
